  ...
  describe 'ユーザー新規登録' do
    ...
    context 'ユーザー登録ができない時' do
      ...
      it 'パスワードが空では登録できない' do
        @user.password = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Password can't be blank", 'Password パスワードは６文字以上で入力してください',
                                                      'Password パスワードは英数字混合にしてください', "Password confirmation doesn't match Password")
      end
      it 'パスワードは６文字以上でないと登録できないこと' do
        @user.password = '12345'
        @user.valid?
        expect(@user.errors.full_messages).to include("Password confirmation doesn't match Password",
                                                      'Password is too short (minimum is 6 characters)', 'Password パスワードは６文字以上で入力してください', 'Password パスワードは英数字混合にしてください')
      end
      it 'パスワードは英語のみでは登録できないこと' do
        @user.password = 'abcdef'
        @user.valid?
        expect(@user.errors.full_messages).to include("Password confirmation doesn't match Password",
                                                      'Password パスワードは英数字混合にしてください')
      end
      ...
      it 'katakana_last_nameが空では登録できない' do
        ...
        expect(@user.errors.full_messages).to include("Katakana last name can't be blank",
                                                      'Katakana last name お名前カナ（全角）は全角カタカナで入力してください')
      end
      ...
      it 'katakana_first_nameが空では登録できない' do
        ...
        expect(@user.errors.full_messages).to include("Katakana first name can't be blank",
                                                      'Katakana first name お名前カナ（全角）は全角カタカナで入力してください')
      end
